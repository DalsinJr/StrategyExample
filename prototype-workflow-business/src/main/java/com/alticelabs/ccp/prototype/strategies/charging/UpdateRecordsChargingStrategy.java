...
public class UpdateRecordsChargingStrategy implements INextCommandStrategy {

    @Override
    public List<BusinessCommand> getNextCommands(BusinessStatus businessStatus) {
        return List.of(new BusinessCommand(new GenerateLDRCommand(), PrototypeSteps.GENERATE_LDR_CHARGING.name()));
    }
}
